...
def api_audit_logs(start_date: Optional[str] = None, end_date: Optional[str] = None, actor: Optional[str] = None, action: Optional[str] = None, outcome: Optional[str] = None, limit: int = 100, offset: int = 0):
    audit = get_audit_logger()
    if audit is None:
...
def api_strategy_get(strategy_id: str):
...
<nav class="sidebar">
  ...
  <div class="sidebar-footer" id="verInfo">Loading...</div>
</nav>
<main class="main">
...
var aLimit=50;
var aTotal=0;
function get(p){return fetch(API+p).then(function(r){if(!r.ok)throw Error(r.text());return r.json();});}
...
async function regModel(){var name=document.getElementById("mName").value.trim();var modelId=document.getElementById("mModelId").value.trim();if(!name||!modelId){var e=document.getElementById("addErr");e.textContent="Name and Model ID are required.";e.style.display="block";return;}try{await post_("/models/register",{name:name,provider:document.getElementById("mProv").value,model_id:modelId,api_key:document.getElementById("mApiKey").value,latency_ms_p50:parseInt(document.getElementById("mLat").value)||500,capability:document.getElementById("mCap").value});var s=document.getElementById("addSuccess");s.textContent="Model registered successfully.";s.style.display="block";document.getElementById("addErr").style.display="none";setTimeout(function(){hideAddForm();loadDeployments();loadDashboard();},1000);}catch(e){var err=document.getElementById("addErr");err.textContent="Error: "+e.message;err.style.display="block";}}
async function deleteModel(name){if(!confirm("Delete model \""+name+"\"?"))return;try{await del__("/models/"+encodeURIComponent(name));loadDeployments();loadDashboard();}catch(e){alert("Error: "+e.message);}}
async function testModel(name){try{await post_("/models/"+encodeURIComponent(name)+"/test",{});alert("Test passed for "+name);}catch(e){alert("Test failed: "+e.message);}}
async function resetCb(name){try{await post_("/circuit-breakers/"+encodeURIComponent(name)+"/reset",{});loadDashboard();}catch(e){alert("Error: "+e.message);}}
async function loadStrats(){try{allStrats=await get("/strategies");renderStratList("");}catch(e){document.getElementById("strList").innerHTML='<div class="empty">'+esc(e.message)+'</div>';}}
...
async function loadCompliance(){updateRetDisplay();try{var lic=await get("/compliance/license");var ls=document.getElementById("licStatus");if(lic.licensed){ls.innerHTML='<div class-badge-green";ls.innerHTML="<div><strong>Licensed to:</strong> "+esc(lic.customer_id||"?")+"</div><div><strong>Plan:</strong> "+esc(lic.plan||"?")+"</div><div><strong>Seats:</strong> "+(lic.seats||"?")+"</div><div><strong>Expiry:</strong> "+esc(lic.expiry||"?")+"</div>";}else{ls.innerHTML='<div class="alert-error">No license installed or invalid.</div>';}}catch(e){document.getElementById("licStatus").innerHTML='<div class="alert-error">Error loading license: '+esc(e.message)+'</div>';}try{var v=await get("/api/version");document.getElementById("sysInfo").innerHTML='<div class="ret-display"><div class=row><span>ModelFungible</span><span class=ival>'+esc(v.modelfungible||"?")+'</span></div><div class=row><span>Python</span><span class=ival>'+esc(v.python||"?")+'</span></div></div>';}catch(e){document.getElementById("sysInfo").innerHTML='<div class="empty">Could not load system info.</div>';}}
